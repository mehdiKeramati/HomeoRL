...
#---------------------   Update : Using SARSA algorithm
def updateQ_SARSA(state,action,nextState,nextAction,reward):
    delta = reward + gamma*Q[nextState*actionsNum + nextAction] - Q[state*actionsNum + action] 
    Q[state*actionsNum + action] = Q[state*actionsNum + action] + alpha*delta
    return 
...
for run in range(0,runsNum):
    ...
    h = initialInState
    nextA = actionSelectionSoftmax(s)
    ...
    while (trial<trialsNum):
        ...
        nextA = actionSelectionSoftmax(nextS)
        
        logging(trial,h,s,a,o,r)    

        updateQ_SARSA(s,a,nextS,nextA,r)            
