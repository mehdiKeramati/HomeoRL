...
#---------------------   Action Selection: SoftMax
def actionSelectionEpsilonGreedySoftmax(state):
...
def updateQ_TD0(state,action,nextState,reward):
    delta = reward - Q[state*actionsNum + action] 
...
def logging(trial,inState,exState,action,outcome,reward,exStateNext,delta):
    ...
    aveReward[trial][action] = aveReward[trial][action] + reward
    aveDelta[trial][action] = aveDelta[trial][action] + delta
...
#-----------
    ...
    A0 = ax1.plot(aveActionT[0] , linewidth = 2 , color='red' )
    A1 = ax1.plot(aveActionT[1] , linewidth = 2 , color='blue' )
    pylab.yticks(pylab.arange(0, 1.01, 0.2))
    pylab.ylim((-0.03,1.03))
    pylab.xlim((-5,trialsNum))
    pylab.xticks(pylab.arange(0, trialsNum+1, 100))
...
#    pylab.set_title('a sine wave')
    fig1.savefig('fMRI_actions.eps', format='eps')
...
#-----------
    ...
    pylab.xticks(pylab.arange(0, trialsNum+1, 100))
    pylab.savefig('fMRI_interalState.eps', format='eps')
...
#    pylab.ylim((-0.03,1.03))
    ...
    for line in ax1.yaxis.get_ticklines():
        line.set_markeredgewidth(1)
    pylab.savefig('fMRI_reward.eps', format='eps')
#-----------
    fig1 = pylab.figure( figsize=(2,1.5) )
    fig1.subplots_adjust(bottom=0.2)
    ax1 = fig1.add_subplot(111)
...
#    pylab.ylim((-0.03,1.03))
    ...
    for line in ax1.yaxis.get_ticklines():
        line.set_markeredgewidth(1)
    pylab.savefig('fMRI_delta.eps', format='eps')

#-----------
    fig1 = pylab.figure( figsize=(2,1.5) )
    fig1.subplots_adjust(bottom=0.2)
    ax1 = fig1.add_subplot(111)
    q0 = ax1.plot(aveQT[0], linewidth = 2 , color = 'red')
    q1 = ax1.plot(aveQT[1], linewidth = 2 , color = 'blue')
#    pylab.yticks(pylab.arange(0, 1.01, 0.2))
#    pylab.ylim((-0.03,1.03))
    pylab.xlim((-5,trialsNum))
    pylab.xticks(pylab.arange(0, trialsNum+1, 100))
    for line in ax1.yaxis.get_ticklines():
        line.set_markeredgewidth(1)
    pylab.savefig('fMRI_Q.eps', format='eps')
...
aveReward = numpy.zeros([trialsNum, actionsNum] , float)
aveDelta = numpy.zeros([trialsNum, actionsNum] , float)
...
for run in range(0,runsNum):
    ...
    while (trial<trialsNum):
        s = 0
#        a = actionSelectionEpsilonGreedy(s)
#        a = actionSelectionEpsilonGreedySoftmax(s)
