...
def seeking (sessionNum):
    ...
    for trial in range(trialCount,trialCount+seekingTrialsNum):
        ...
        if sessionNum == 1:
            updateOutcomeFunction           ( exState , action , nextState , out/2        )
        else:
...
def plotInternalStateOral():
    ...
    S0 = ax1.plot(internalStateLog [0:totalTrialsNum/2] , linewidth = 2 , color='black' )

    ax1.axhline(50, color='0.25',ls='--', lw=1 )

    pylab.yticks(pylab.arange(0, 51, 10))
    pylab.ylim((-10,60))
    pylab.xlim( ( -50  , totalTrialsNum/2 +50 ) )
...
def plotInternalStateFistula():
    ...
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(internalStateLog [totalTrialsNum/2 : totalTrialsNum] , linewidth = 2 , color='black' )
    ...
    ax1.set_ylabel('Internal State')
    ax1.set_xlabel('Time (min)')
    ax1.set_title('Oral/Fistula reward')
...
def plotEtimatedWaterProbabilityOral():
    ...
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(estimatedOutcomeOnGreenLog  [0 : totalTrialsNum/2] , linewidth = 2 , color='black' )
    ...
    ax1.set_ylabel('Subjective water magnitude')
    ax1.set_xlabel('Time (min)')
    ax1.set_title('Oral reward')
...
def plotEtimatedWaterProbabilityFistula():
    ...
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(estimatedOutcomeOnGreenLog  [totalTrialsNum/2 : totalTrialsNum] , linewidth = 2 , color='black' )
...
def plotLeverPressPer5MinOral():
    ...
    greenKey  = numpy.zeros( [2*8] , float)
    yellowKey = numpy.zeros( [2*8] , float)
    ...
    S1 = ax1.plot(x  ,greenKey[0:8]    , '-o', ms=7, markeredgewidth =0, alpha=1, mfc='black',linewidth = 2 , color='black' )    

    pylab.yticks(pylab.arange(10, 46, 10))
    pylab.ylim((10,47))
    pylab.xlim((0,45))
...
def plotLeverPressPer5MinFistula():
    ...
    S1 = ax1.plot(x  , greenKey[8:16]    , '-o', ms=7, markeredgewidth =0, alpha=1, mfc='black',linewidth = 2 , color='black' )
...
pretrainingHours    = 0
sessionsNum         = 2                                # Number of sessions of water seeking, followed by rest in home-cage
seekingHours        = 0.66           
