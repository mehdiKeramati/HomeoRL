...
#-----------
    ...
    S1 = ax1.plot(aveActionT[1] , linewidth = 2 , color='green' )
    S2 = ax1.plot(aveActionT[2] , linewidth = 2 , color='blue' )
    pylab.yticks(pylab.arange(0, 1.01, 0.2))
    pylab.ylim((-0.03,1.03))
    pylab.xlim((-5,400))
    pylab.xticks(pylab.arange(0, 401, 100))
...
#    pylab.set_title('a sine wave')
    fig1.savefig('Preferences.eps', format='eps')

#-----------
    line = numpy.zeros([trialsNum] , float)
    for i in range(0,trialsNum):        
         line[i] = 0
    ...
    fig1.subplots_adjust(bottom=0.2)
    ax1 = fig1.add_subplot(111)
    l = ax1.plot(line , linewidth = 1.5 , color = 'gray', linestyle = 'dashed' )
    h = ax1.plot(aveInStateT , linewidth = 2 , color = 'black')
    pylab.yticks(pylab.arange(-50, 51, 25))
    pylab.xticks(pylab.arange(0, 401, 100))
